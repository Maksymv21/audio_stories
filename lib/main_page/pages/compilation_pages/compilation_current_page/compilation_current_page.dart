...
import 'compilation_current_bloc/compilation_current_bloc.dart';
import 'compilation_current_bloc/compilation_current_state.dart';
  ...
  Widget _soundList(List listId, String id) {
    return StreamBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.hasData) {
          final int _length = listId.length;
          _createLists(snapshot, _length);

          if (listTitle.isEmpty || listTitle.length < _length) {
            listTitle = List.filled(_length, '');
            listUrl = List.filled(_length, '');
            listTime = List.filled(_length, 0.0);
            for (int i = 0; i < snapshot.data.docs.length; i++) {
              for (int j = 0; j < _length; j++) {
                if (snapshot.data.docs[i].id == listId[j]) {
                  listTitle[j] = snapshot.data.docs[i]['title'];
                  listUrl[j] = snapshot.data.docs[i]['song'];
                  listTime[j] = snapshot.data.docs[i]['time'];
                }
              }
            }
          }
